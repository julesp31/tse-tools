    ...
    for word in list_of_words:
        ...
        if busy_boolean:
            ...
            if square_bracket_count > 0:
                ...
                busy_boolean = False
                busy_list_complete = True
                break

        if busy_list_complete:
            break
...
def get_blocks(busy_list):
    ...
    iterator = 1
    for block in blocks:
        print("Busy " + str(iterator) + ' ' + str(block))
        print()
        print()
        iterator += 1
    '''
...
def get_busy_times(blocks):
    ...
    for block in blocks:
        ...
        busy_times.append(f"{starting_time}|{ending_time}")

    #print(f"Original Busy Times: {busy_times}")
    #print()
    return combine_busy_times(busy_times)


def combine_busy_times(busy_times):
    if not busy_times:
        return []

    # Step 1: Parse and sort the time blocks
    intervals = [parse_range(t) for t in busy_times]
